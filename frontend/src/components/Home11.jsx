...
function Home11(props) {
    function handleSubmit(event){
        ...
        fetch("http://192.168.34.141:8000" + "/teacherLogin", { // sends a POST request to the server with the form data
            method: "POST",
            headers: {
        ...
        .then((recvData) => {               // handles the JSON response from the server
            if(recvData.success === "success"){
                props.invertLog();
                props.getUsername(recvData.data);
        ...
        <div className="bigHead" style={{marginTop:"50px"}}>
            ...
            <img className="img1" src="images/sssihl.png"/>
                ...
                <div className="row">
                    ...
                    <Link to="/Login22"><button className="teacherStu" style={{padding:"5px",marginLeft:"5px",height:"30px", width:"80px"}}> STUDENT </button></Link>
                </div> <br />
            </div>
        </div>
        </>
